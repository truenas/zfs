 ...
 * SA_ZPL_SEQ in its layout. Caller's bulk MUST include SA_ZPL_FLAGS so the
 * ZFS_HAS_SEQ bit reaches disk. Caller MUST clear ZFS_HAS_SEQ on bulk update
 * failure (zfs_persist_seq_rollback).
 */

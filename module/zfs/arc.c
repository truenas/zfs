    ...
    uint64_t sublist_headroom, uint64_t guid)
{
	...
	/*
	 * Until the ARC is warm and starts to evict, read from the head of the
	 * ARC lists rather than the tail.
	 */
	if (from_head)
...
skip:
		...
		*consumed += HDR_GET_LSIZE(hdr);
		...
		/*
		 * If the allocated size of this buffer plus the max size for
		 * the pending log block exceeds the evicted target size,
		 * terminate writing buffers for this run.
		 */
		if (*write_asize + asize + sizeof (l2arc_log_blk_phys_t) >
		    target_sz) {
			full = B_TRUE;
			mutex_exit(hash_lock);
			break;
		...
		/*
		 * We should not sleep with sublist lock held or it may block
		 * ARC eviction.  Insert a marker to save the position and drop
		 * the lock.
		 */
		if (from_head) {
		...
		/*
		 * If this header has b_rabd, we can use this since it must
		 * always match the data exactly as it exists on disk.
		 * Otherwise, the L2ARC can normally use the hdr's data, but if
		 * we're sharing data between the hdr and one of its bufs, L2ARC
		 * needs its own copy of the data so that the ZIO below can't
		 * race with the buf consumer. To ensure that this copy will be
		 * available for the lifetime of the ZIO and be cleaned up
		 * afterwards, we add it to the l2arc_free_on_write queue. If we
		 * need to apply any transforms to the data (compression,
		...
		if (*pio == NULL) {
			 ...
			 * l2arc_write_done() can find where the write buffers
			 * begin without searching.
			 */
		...
		zio_t *wzio = zio_write_phys(*pio, dev->l2ad_vdev,
		    dev->l2ad_hand, asize, to_write, ZIO_CHECKSUM_OFF, NULL,
		    hdr, ZIO_PRIORITY_ASYNC_WRITE, ZIO_FLAG_CANFAIL, B_FALSE);

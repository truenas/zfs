...
/*
 * Switch taskq at multiple of 128 MB offset. This can be set to a lower value
 * to utilize more threads for small files but may affect prefetch hits.
 */
#define	ZVOL_TASKQ_OFFSET_SHIFT 27
...
zvol_init(void)
{
	...
	ztqs->tqs_taskq = kmem_alloc(num_tqs * sizeof (taskq_t *),
	    KM_SLEEP);

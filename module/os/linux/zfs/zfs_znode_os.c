...
zfs_rezget(znode_t *zp)
{
	 ...
	 * while SA_ZPL_SEQ is gone on disk. Only the marker is recomputed;
	 * z_seq is left as-is so the change cookie does not step backward
	 * across a rollback (it stays monotonic, refreshed on next modify).
	 * The lookup reads a throwaway buffer purely to test presence.
	 */
	uint64_t tmp_seq;

...
zfs_deleteextattr_impl(struct vop_deleteextattr_args *ap, boolean_t compat)
{
	...
	size_t size = 0;
	struct vop_getextattr_args gap = {
		.a_vp = ap->a_vp,
		.a_size = &size,
		.a_cred = ap->a_cred,
		.a_td = ap->a_td,
	...
	error = ENOENT;
	if (zfsvfs->z_use_sa && zp->z_is_sa) {
		error = zfs_getextattr_sa(&gap, attrname);
		if (error == 0) {
			error = zfs_deleteextattr_sa(ap, attrname);
		}
	}
	if (error == ENOENT) {
		error = zfs_getextattr_dir(&gap, attrname);
		if (error == 0) {
...
zfs_setextattr_impl(struct vop_setextattr_args *ap, boolean_t compat)
{
	...
	struct vop_deleteextattr_args dap = {
		.a_vp = ap->a_vp,
		.a_cred = ap->a_cred,
		.a_td = ap->a_td,
	};
	...
	if (zfsvfs->z_use_sa && zp->z_is_sa && zfsvfs->z_xattr_sa) {
		...
		if (error == 0) {
			 ...
			 * in dir if present.
			 */
			zfs_deleteextattr_dir(&dap, attrname);
		}
	}
	if (error) {
		...
		if (error == 0) {
			 ...
			 * in SA if present.
			 */
			zfs_deleteextattr_sa(&dap, attrname);
		}
	}
		...
		for (int pos = 0; pos < done; ) {
			...
			if (sizep != NULL) {
				*sizep += 1 + nlen;
				 ...
				 */
				error = uiomove(&nlen, 1, uiop);
				if (error == 0) {
					char *namep = dp->d_name + plen;
					error = uiomove(namep, nlen, uiop);
				}
				if (error != 0) {
					break;
				}

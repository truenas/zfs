 ...
 */
static void
diff_json_add_changes(nvlist_t *nvl, zfs_diff_entry_t *entry, boolean_t timestamped)
{
	...
	if (entry->de_type == ZFS_DIFF_RENAMED)
		fnvlist_add_string(changes, "path", entry->de_path2);
...
diff_json_callback(zfs_diff_entry_t *entry, void *arg)
{
	...
	fnvlist_add_string(nvl, "path", entry->de_path);

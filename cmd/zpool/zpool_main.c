...
fill_vdev_info(nvlist_t *list, zpool_handle_t *zhp, char *name,
    boolean_t addtype)
{
	boolean_t l2c, log, spare;
	const char *path, *phys, *devid;
	uint64_t hole = 0;
	vdev_stat_t *vs;
	uint_t c;
	l2c = log = spare = B_FALSE;
	...
	if (nvdev) {
		...
		if (log)
			fnvlist_add_string(list, ZPOOL_CONFIG_IS_LOG, "1");
		if (spare)
			fnvlist_add_string(list, ZPOOL_CONFIG_IS_SPARE, "1");
		if (nvlist_lookup_uint64(nvdev, ZPOOL_CONFIG_IS_HOLE,
		    &hole) == 0)
			fnvlist_add_string(list, ZPOOL_CONFIG_IS_HOLE, "1");
    ...
    nvlist_t *item)
{
	...
	char tmp[256];
	const char *val;
	nvlist_t *command = fnvlist_alloc();
	...
	for (i = 0; i < vcdl->count; i++) {
		...
		for (j = 0; j < vcdl->uniq_cols_cnt; j++) {
			...
			if (val == NULL || is_blank_str(val))
				val = "-";
			fnvlist_add_string(command, vcdl->uniq_cols[i], val);
		}

		for (j = data->cols_cnt; j < data->lines_cnt; j++) {
			if (data->lines[j]) {
				snprintf(tmp, 256, "extra_%d", k++);
				fnvlist_add_string(command, tmp,
				    data->lines[j]);
			}
		}
		break;
	}
	if (!nvlist_empty(command))
		fnvlist_add_nvlist(item, vcdl->cmd, command);
	fnvlist_free(command);
}
...
collect_pool(zpool_handle_t *zhp, list_cbdata_t *cb)
{
	...
	if (cb->cb_json) {
		item = fnvlist_alloc();
		props = fnvlist_alloc();
		d = fnvlist_lookup_nvlist(cb->cb_jsobj, "data");
		if (d == NULL) {
    ...
    list_cbdata_t *cb, int depth, boolean_t isspare, nvlist_t *item)
{
	...
	if (name != NULL) {
		...
		collect_vdev_prop(ZPOOL_PROP_HEALTH, 0, state, scripted,
		    B_TRUE, format, cb->cb_json, props);
	...
	if (cb->cb_json) {
		if (!nvlist_empty(ch))
			fnvlist_add_nvlist(ent, "children", ch);
		fnvlist_free(ch);
	}
...
zpool_do_list(int argc, char **argv)
{
	...
	/* check options */
	while ((c = getopt(argc, argv, ":gjHLo:pPT:v")) != -1) {
		switch (c) {
		case 'g':
	...
	for (;;) {
		...
		if (timestamp_fmt != NODATE) {
			if (cb.cb_json) {
				...
				fnvlist_add_string(cb.cb_jsobj, "time", ts);
			} else
				print_timestamp(timestamp_fmt);
		}
...
vdev_stats_nvlist(zpool_handle_t *zhp, status_cbdata_t *cb, nvlist_t *nv,
    int depth, boolean_t isspare, nvlist_t *item)
{
	...
	vname = zpool_vdev_name(g_zfs, zhp, nv,
	    cb->cb_name_flags | VDEV_NAME_TYPE_ID);
	vds = fnvlist_alloc();
	fill_vdev_info(vds, zhp, vname, B_FALSE);
	if (!isspare) {
		if (vs->vs_alloc) {
			nice_num_str_nvlist(vds, "alloc_space", vs->vs_alloc,
			    cb->cb_literal, ZFS_NICENUM_BYTES);
		}
		if (vs->vs_space) {
		...
		if (vs->vs_resilver_deferred) {
			nice_num_str_nvlist(vds, "resilver_deferred",
			    vs->vs_resilver_deferred, B_TRUE, ZFS_NICENUM_1024);
		}
		if (cb->cb_print_slow_ios) {
			if (children == 0) {
				nice_num_str_nvlist(vds, "slow_ios",
				    vs->vs_slow_ios, cb->cb_literal,
				    ZFS_NICENUM_1024);
			}
			else
				fnvlist_add_string(vds, "slow_ios", "-");
		}
		if (cb->cb_print_power) {
			if (children == 0)  {
				/* Only leaf vdevs have physical slots */
	...
	if (nvlist_lookup_uint64(nv, ZPOOL_CONFIG_NOT_PRESENT,
	    &notpresent) == 0) {
		fnvlist_add_string(vds, ZPOOL_CONFIG_NOT_PRESENT, "1");
		...
		nice_num_str_nvlist(vds, "configured_ashift",
		    vs->vs_configured_ashift, B_TRUE, ZFS_NICENUM_1024);
		nice_num_str_nvlist(vds, "physical_ashift",
		    vs->vs_physical_ashift, B_TRUE, ZFS_NICENUM_1024);
	}
	if (vs->vs_scan_removing != 0) {
		nice_num_str_nvlist(vds, "removing", vs->vs_scan_removing,
		    B_TRUE, ZFS_NICENUM_1024);
	} else if (VDEV_STAT_VALID(vs_noalloc, vsc) && vs->vs_noalloc != 0) {
	...
	if (children == 0) {
		if (cb->cb_print_vdev_init) {
			if (vs->vs_initialize_state != 0) {
				...
				nice_num_str_nvlist(vds, "init_time",
				    ...
				    cb->cb_literal, ZFS_NICE_TIMESTAMP);
		...
		if (cb->cb_print_vdev_trim) {
			...
			nice_num_str_nvlist(vds, "trim_notsup",
			    vs->vs_trim_notsup, B_TRUE, ZFS_NICENUM_1024);
		}
	} else {
		ch = fnvlist_alloc();
	}
	...
	for (int c = 0; c < children; c++) {
		...
		if (nvlist_exists(child[c], ZPOOL_CONFIG_ALLOCATION_BIAS))
			continue;
		vdev_stats_nvlist(zhp, cb, child[c], depth + 2, isspare, ch);
	}

	if (ch != NULL) {
		if (!nvlist_empty(ch))
			fnvlist_add_nvlist(vds, "children", ch);
		fnvlist_free(ch);
	}
    ...
    const char *class, nvlist_t *item)
{
	uint_t c, children;
	nvlist_t **child;
	nvlist_t *class_obj = fnvlist_alloc();
	...
	for (c = 0; c < children; c++) {
		...
		vdev_stats_nvlist(zhp, cb, child[c], 2, B_FALSE, class_obj);
		free(name);
	}
    ...
    nvlist_t *item)
{
	...
	if (nvlist_lookup_nvlist_array(nv, ZPOOL_CONFIG_L2CACHE,
		...
		for (int i = 0; i < nl2cache; i++)
			vdev_stats_nvlist(zhp, cb, l2cache[i], 2, B_FALSE, l2c);
	}
	if (!nvlist_empty(l2c))
		fnvlist_add_nvlist(item, "l2cache", l2c);
	fnvlist_free(l2c);
    ...
    nvlist_t *item)
{
	...
	if (nvlist_lookup_nvlist_array(nv, ZPOOL_CONFIG_SPARES,
		...
		for (int i = 0; i < nspares; i++)
			vdev_stats_nvlist(zhp, cb, spares[i], 2, B_TRUE, sp);
	}
	if (!nvlist_empty(sp))
		fnvlist_add_nvlist(item, "spares", sp);
	fnvlist_free(sp);
    ...
    nvlist_t *nvroot, nvlist_t *item)
{
	...
	if (nvlist_lookup_uint64_array(nvroot,
		...
		nice_num_str_nvlist(nv, "expanding_vdev",
		    pres->pres_expanding_vdev, B_TRUE, ZFS_NICENUM_1024);
		...
		nice_num_str_nvlist(nv, "waiting_for_resilver",
		    pres->pres_waiting_for_resilver, B_TRUE, ZFS_NICENUM_1024);
    ...
    nvlist_t *item)
{
	...
	if (nvlist_lookup_uint64_array(nvroot,
		...
		nice_num_str_nvlist(nv, "start_time",
		    pcs->pcs_start_time, cb->cb_literal, ZFS_NICE_TIMESTAMP);
		nice_num_str_nvlist(nv, "space",
		    pcs->pcs_space, cb->cb_literal, ZFS_NICENUM_BYTES);
	...
	if (nvlist_lookup_uint64_array(nvroot, ZPOOL_CONFIG_REMOVAL_STATS,
		...
		if (prs->prs_state != DSS_NONE) {
			...
			nice_num_str_nvlist(nv, "removing_vdev",
			    prs->prs_removing_vdev, B_TRUE, ZFS_NICENUM_1024);
			nice_num_str_nvlist(nv, "start_time",
    ...
    nvlist_t *nvroot, nvlist_t *item)
{
	...
	if (nvlist_lookup_uint64_array(nvroot, ZPOOL_CONFIG_SCAN_STATS,
		...
		nice_num_str_nvlist(scan, "issued_bytes_per_scan",
		    ps->pss_pass_issued, cb->cb_literal, ZFS_NICENUM_BYTES);
			...
			if (nvlist_lookup_uint64_array(child[c],
				...
				if (vrs->vrs_state != VDEV_REBUILD_NONE) {
					...
					nice_num_str_nvlist(nv, "scan_time",
					    ...
					    cb->cb_literal, ZFS_NICENUM_TIME);
...
status_callback_json(zpool_handle_t *zhp, void *data)
{
	...
	item = fnvlist_alloc();
	vds = fnvlist_alloc();
	fill_pool_info(item, zhp, B_FALSE);
	config = zpool_get_config(zhp, NULL);
	...
	if (config != NULL) {
		...
		vdev_stats_nvlist(zhp, cbp, nvroot, 0, B_FALSE, vds);
		fnvlist_add_nvlist(item, ZPOOL_CONFIG_VDEV_STATS, vds);
		fnvlist_free(vds);

...
collect_pool(zpool_handle_t *zhp, list_cbdata_t *cb)
{
	...
	if (cb->cb_json) {
		...
		fnvlist_add_nvlist(d, zpool_get_name(zhp), item);
		fnvlist_free(props);
		fnvlist_free(item);
	} else
		(void) fputc('\n', stdout);
...
zpool_do_list(int argc, char **argv)
{
	...
	struct option long_options[] = {
		{"int", no_argument, NULL, ZPOOL_OPTION_JSON_NUMS_AS_INT},
		{0, 0, 0, 0}
	};
	...
	if (!cb.cb_json && cb.cb_json_as_int) {
		(void) fprintf(stderr, gettext("'--int' only works with '-j'"
		    " option\n"));
		usage(B_FALSE);
	}
...
zpool_do_status(int argc, char **argv)
{
	...
	struct option long_options[] = {
		{"power", no_argument, NULL, ZPOOL_OPTION_POWER},
		{"int", no_argument, NULL, ZPOOL_OPTION_JSON_NUMS_AS_INT},
		{"flat", no_argument, NULL, ZPOOL_OPTION_JSON_FLAT_VDEVS},
		{0, 0, 0, 0}
	};
	...
	if (cb.cb_flat_vdevs && !cb.cb_json) {
		fprintf(stderr, "'--flat' only works with '-j' option\n");
		usage(B_FALSE);
	}

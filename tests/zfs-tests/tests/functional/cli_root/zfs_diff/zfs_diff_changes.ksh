...
log_must eval "zfs diff -j $TESTSNAP1 $TESTSNAP2 > $FILEDIFF"
nlink_from="$(jq -r --arg p "$MNTPOINT/flinked" \
    'select(.path == $p) | .changes.nlink.from' "$FILEDIFF" 2>/dev/null)"
nlink_to="$(jq -r --arg p "$MNTPOINT/flinked" \
...
ctime_from="$(jq -r --arg p "$MNTPOINT/fmodified" \
    'select(.path == $p) | .changes.ctime.from' "$FILEDIFF" 2>/dev/null)"
ctime_to="$(jq -r --arg p "$MNTPOINT/fmodified" \
...
ctime_from="$(jq -r --arg p "$MNTPOINT/fcreated" \
    'select(.path == $p) | .changes.ctime.from' "$FILEDIFF" 2>/dev/null)"
ctime_to="$(jq -r --arg p "$MNTPOINT/fcreated" \
    'select(.path == $p) | .changes.ctime.to' "$FILEDIFF" 2>/dev/null)"
...
ctime_from="$(jq -r --arg p "$MNTPOINT/fremoved" \
    'select(.path == $p) | .changes.ctime.from' "$FILEDIFF" 2>/dev/null)"
ctime_to="$(jq -r --arg p "$MNTPOINT/fremoved" \
    'select(.path == $p) | .changes.ctime.to' "$FILEDIFF" 2>/dev/null)"

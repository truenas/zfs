...
#	6. Fill L2ARC again to complete first pass.
#	7. Measure DWPD-limited writes again.
#	8. Verify rate limiting persists after import (similar write rates).
#
...
# Fill first pass again after import and wait for L2ARC writes to stabilize
...
# Verify rate limiting persists after import (writes should be similar)
# Require after-import writes to be within 75-125% of before-export writes
if [[ $writes_after -eq 0 ]]; then
	log_fail "No writes after import - rate limiting may be broken"
fi

typeset min_writes=$((writes_before * 75 / 100))
typeset max_writes=$((writes_before * 125 / 100))
if [[ $writes_after -lt $min_writes ]] || [[ $writes_after -gt $max_writes ]]; then
	log_fail "Writes after import ($((writes_after / 1024))KB) outside expected range" \
	    "[$((min_writes / 1024))KB - $((max_writes / 1024))KB]" \
	    "(before export: $((writes_before / 1024))KB)"
fi
